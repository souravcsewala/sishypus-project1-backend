                ...
                // 5. delete instructor by admin -- admin power
                 ...
                 const UpdateCard = async (req, res, next) => {
                  ...
                  // 9. delete a card byadmin --admin power 
                   ...
                   const CardDelete=async(req,res,next)=>{
                             ...
                             try{
                                  const GetList= await FreeconsultancyModel.find({});
                                     if(!GetList || GetList.length === 0){
                                        return next(new ErrorHandeler("No free consultancy found or list is empty",404));
                                     }
                                        res.status(200).send({
                     ...
                     // 11. post upcoming events 
                        ...
                        const{title,eventDate,description}=req.body
                                if(!title || !eventDate || !description || !eventDate.day || !eventDate.month || !eventDate.year){
                                      return next (new ErrorHandeler("please provide all details", 400));
                                }
                                    const newEvent=await UpcomingEventsModel.create({title,eventDate,description})
                                           if(!newEvent){
                     ...
                     // 13.  delete upcoming events 
                        ...
                        const EventRemove= await UpcomingEventsModel.deleteOne({_id:req.params.id})
                                      res.status(200).json({
                   ...
                   return next(new ErrorHandeler("Bad Request: Missing required fields please provide all details",400))
                             }
                     ...
                     // 17. create german course 
                          const CreateGermanCourse=async(req,res,next)=>{
                                try{
                 const{title,description,totalclass,duration,discount,price}=req.body
                                if(!title || !description){
                             return next(new ErrorHandeler("please provide title and description",400))
                                }
                              const CourseCreate= await GermanCourseModel.create({title,description,totalclass,duration,discount,price})
                              res.status(201).json({
                                 success:true,
                ...
                const FindGermanCourse= await GermanCourseModel.findById(req.params.id)
                    ...
                    await FindGermanCourse.remove(); // here i can use deleteOne and findByIdAndRemove 
                     ...
                     const CreateExcelCourse=async(req,res,next)=>{
                      try{
       const{title,weeklyclass,totalclass,duration,discount,price}=req.body
                      if(!title){
                   return next(new ErrorHandeler("please provide title ",400))
                      }
                    const CourseCreate= await ExcelCourseModel.create({title,weeklyclass,totalclass,duration,discount,price})
                    res.status(201).json({
                       success:true,
                ...
                const FindExcelCourse= await ExcelCourseModel.findById(req.params.id)
                   ...
                   return next(new ErrorHandeler("please provide all details ",400))
                    ...
                    const WebniarCreate= await ExcelWebniarModel.create({day,date,month,time,webniarurl})
                    res.status(201).json({
                       success:true,
                ...
                const FindExcelWebniar= await ExcelWebniarModel.findById(req.params.id)
                  ...
                  if (!FindExcelWebniarRegisterList || FindExcelWebniarRegisterList.length === 0) {
                    return next(new ErrorHandeler(" Excel Webniar  register list empty", 401));
                }
                res.status(200).json({
                     success:true,
